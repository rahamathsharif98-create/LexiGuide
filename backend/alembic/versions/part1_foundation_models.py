"""Alembic migration for Part 1: child_profiles, child_interests, child_comfort_preferences, child_language_profiles, child_learning_profiles."""
import sqlalchemy as sa
from alembic import op

# revision identifiers, used by Alembic.
revision = 'part1_foundation_models'
down_revision = None
branch_labels = None
depends_on = None

def upgrade() -> None:
    # 1. ChildProfile (Explicit child account profile table)
    op.create_table(
        'child_profiles',
        ...
        sa.Column('age_group', sa.String(20), nullable=True),  # e.g. "early-reader", "developing"
        sa.Column('grade', sa.String(20), nullable=True),      # optional
        sa.Column('avatar', sa.String(20), nullable=False, server_default='🦊'),
        sa.Column('mother_tongue', sa.String(10), nullable=False, server_default='en'),
        sa.Column('learning_languages', sa.JSON(), nullable=False),
        sa.Column('interface_language', sa.String(10), nullable=False, server_default='en'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), onupdate=sa.func.now(), nullable=True),
    )

    # 2. ChildInterest (Presentation themes only)
    op.create_table(
        'child_interests',
        sa.Column('id', sa.Integer(), primary_key=True, index=True),
        sa.Column('student_id', sa.Integer(), sa.ForeignKey('students.id', ondelete='CASCADE'), nullable=False, unique=True, index=True),
    ...
    # 3. ChildComfortPreference (Sensory ergonomics)
    op.create_table(
    ...
    # 4. LanguageProfile (Multilingual bridge)
    op.create_table(
    ...
    # 5. LearningProfile (Strictly separate evidence profile)
    op.create_table(
