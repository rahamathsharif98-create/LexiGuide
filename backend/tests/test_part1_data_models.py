"""Unit tests for Part 1: Child Profile, Interests, Comfort, Language, and Evidence-Based Learning Profile models."""
import unittest
...
class TestPart1DataModels(unittest.TestCase):
    def setUp(self):
    ...
    def test_child_profile_model(self):
        profile = ChildProfile(
            ...
            avatar="🦊",
            mother_tongue="te",
            learning_languages=["en", "te"],
            interface_language="en",
        )
        self.db.add(profile)
        self.db.commit()

        retrieved = self.db.query(ChildProfile).filter_by(student_id=self.student.id).first()
        self.assertIsNotNone(retrieved)
        self.assertEqual(retrieved.preferred_name, "Aarav")
        self.assertEqual(retrieved.grade, "Grade 2")
        self.assertEqual(retrieved.mother_tongue, "te")
        self.assertIn("en", retrieved.learning_languages)
    ...
    def test_cascade_delete_on_student(self):
        ...
        self.db.add(ChildProfile(student_id=self.student.id, preferred_name="Aarav"))
        ...
        self.assertIsNone(self.db.query(ChildProfile).filter_by(student_id=self.student.id).first())
